...
@section('content')
    ...
    <div class="mt-6 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
        <form method="POST" action="{{ route('project-categories.update', $data->uuid) }}" class="space-y-4">
            @csrf
            @method('PUT')
            <div>
            ...
            <div>
                ...
                <textarea id="description" name="description" class="mt-2 w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-200" placeholder="Category Description">{{ old('description', $data->description) }}</textarea>
                @error('description')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
