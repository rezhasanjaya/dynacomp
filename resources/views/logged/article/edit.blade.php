...
@section('content')
    ...
    <div class="mt-6 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
        <form method="POST" action="{{ route('article.update', $data->id) }}" class="space-y-4" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div>
                <label for="title" class="text-sm font-semibold text-slate-700">Title</label>
                <input id="title" name="title" type="text" value="{{ old('title', $data->title) }}" class="mt-2 w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-200" placeholder="Article Title" required>
                @error('title')
            ...
            <div>
                ...
                <select id="article_category_id" name="article_category_id" class="mt-2 w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-200" required>
                    ...
                    @foreach ($category as $item)
                        <option value="{{ $item->id }}" {{ old('article_category_id', $data->article_category_id) == $item->id ? 'selected' : '' }}>
                            {{ $item->category }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input id="author" name="author" type="text" value="{{ old('author', $data->author) }}" class="mt-2 w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-200" placeholder="Author Name">
                @error('author')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            <div>
                ...
                <input id="published_date" name="published_date" type="date" value="{{ old('published_date', $data->published_date) }}" class="mt-2 w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-200">
            ...
            <div>
                ...
                <textarea id="content" name="content" class="mt-2 w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-200" placeholder="Article Content">{{ old('content', $data->content) }}</textarea>
                @error('content')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            <div>
                ...
                @if ($data->image)
                    ...
                    <div class="mt-2">
                        <img src="{{ asset('storage/' . $data->image) }}" alt="Article image" class="h-40 w-64 rounded-lg object-cover ring-1 ring-slate-200">
                    </div>
                @endif
                @error('image')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
